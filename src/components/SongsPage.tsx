      ...
      <div className="space-y-3">
        {songs?.map((song) => (
          <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="bg-card-main rounded-xl border border-border-main overflow-hidden"
          >
            <Link to={`/songs/${song.idtononkira}`} className="p-4 flex items-center gap-4 active:bg-card-main transition-colors">
        ...
        {songs?.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-text-main/50">Aucune chanson trouvée</p>
          </div>
        )}
      </div>
    </div>
